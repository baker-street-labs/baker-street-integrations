        ...
        try {
            if ($groupName -eq "Domain Computers") {
                ...
                if ($group) {
                    ...
                    Write-Host "   ⚠️  Group $groupName not found, skipping..." -ForegroundColor Yellow
                    continue
                }
            }
            ...
            Write-Host "   ⚠️  Failed to set permissions for $groupName : $_" -ForegroundColor Yellow
        }
    }
...
if (Get-ADObject -Filter "cn -eq '$templateName'" -SearchBase $templatesDN -ErrorAction SilentlyContinue) {
    Write-Host "   ⚠️  Template already exists, skipping" -ForegroundColor Yellow
    $skipped++
} else {
    try {
        $attributes = @{
        ...
        Write-Host "   ✅ Created BSLMachineAuth" -ForegroundColor Green
        $created++
    } catch {
...
if (Get-ADObject -Filter "cn -eq '$templateName'" -SearchBase $templatesDN -ErrorAction SilentlyContinue) {
    ...
    try {
        ...
        Write-Host "   ✅ Created BSLUserAuth" -ForegroundColor Green
        $created++
    } catch {
        Write-Host "   ❌ Failed: $_" -ForegroundColor Red
        $failed++
...
if (Get-ADObject -Filter "cn -eq '$templateName'" -SearchBase $templatesDN -ErrorAction SilentlyContinue) {
    ...
    try {
        ...
        Write-Host "   ✅ Created BSLWebServer" -ForegroundColor Green
        $created++
    } catch {
        Write-Host "   ❌ Failed: $_" -ForegroundColor Red
        $failed++
...
if (Get-ADObject -Filter "cn -eq '$templateName'" -SearchBase $templatesDN -ErrorAction SilentlyContinue) {
    ...
    try {
        ...
        New-ADObject -Name $templateName -Type pKICertificateTemplate -Path $templatesDN -OtherAttributes $attributes -ErrorAction Stop
        Start-Sleep -Seconds 1
        Write-Host "   ✅ Created BSLK8sSigner (note: set as CA template in GUI if needed)" -ForegroundColor Green
        $created++
    } catch {
        Write-Host "   ❌ Failed: $_" -ForegroundColor Red
        $failed++
...
if (Get-ADObject -Filter "cn -eq '$templateName'" -SearchBase $templatesDN -ErrorAction SilentlyContinue) {
    ...
    try {
        ...
        New-ADObject -Name $templateName -Type pKICertificateTemplate -Path $templatesDN -OtherAttributes $attributes -ErrorAction Stop
        Start-Sleep -Seconds 1
        Write-Host "   ✅ Created BSLK8sService" -ForegroundColor Green
        $created++
    } catch {
        Write-Host "   ❌ Failed: $_" -ForegroundColor Red
        $failed++
...
if (Get-ADObject -Filter "cn -eq '$templateName'" -SearchBase $templatesDN -ErrorAction SilentlyContinue) {
    ...
    try {
        ...
        Start-Sleep -Seconds 1
        Set-TemplatePermissions -TemplateDN $templateDN -Groups @("Domain Computers")
        Write-Host "   ✅ Created BSLLinuxAuth" -ForegroundColor Green
        $created++
    } catch {
        Write-Host "   ❌ Failed: $_" -ForegroundColor Red
        $failed++
...
if (Get-ADObject -Filter "cn -eq '$templateName'" -SearchBase $templatesDN -ErrorAction SilentlyContinue) {
    ...
    try {
        ...
        New-ADObject -Name $templateName -Type pKICertificateTemplate -Path $templatesDN -OtherAttributes $attributes -ErrorAction Stop
        Start-Sleep -Seconds 1
        Write-Host "   ✅ Created BSLCodeSigning" -ForegroundColor Green
        $created++
    } catch {
        Write-Host "   ❌ Failed: $_" -ForegroundColor Red
        $failed++
...
if (Get-ADObject -Filter "cn -eq '$templateName'" -SearchBase $templatesDN -ErrorAction SilentlyContinue) {
    ...
    try {
        ...
        New-ADObject -Name $templateName -Type pKICertificateTemplate -Path $templatesDN -OtherAttributes $attributes -ErrorAction Stop
        Start-Sleep -Seconds 1
        Write-Host "   ✅ Created BSLVPNAuth" -ForegroundColor Green
        $created++
    } catch {
        Write-Host "   ❌ Failed: $_" -ForegroundColor Red
        $failed++
...
if (Get-ADObject -Filter "cn -eq '$templateName'" -SearchBase $templatesDN -ErrorAction SilentlyContinue) {
    ...
    try {
        ...
        New-ADObject -Name $templateName -Type pKICertificateTemplate -Path $templatesDN -OtherAttributes $attributes -ErrorAction Stop
        Start-Sleep -Seconds 1
        Write-Host "   ✅ Created BSLRedTeam" -ForegroundColor Green
        $created++
    } catch {
        Write-Host "   ❌ Failed: $_" -ForegroundColor Red
        $failed++
...
if (Get-ADObject -Filter "cn -eq '$templateName'" -SearchBase $templatesDN -ErrorAction SilentlyContinue) {
    ...
    try {
        ...
        New-ADObject -Name $templateName -Type pKICertificateTemplate -Path $templatesDN -OtherAttributes $attributes -ErrorAction Stop
        Start-Sleep -Seconds 1
        Write-Host "   ✅ Created BSLShortLived" -ForegroundColor Green
        $created++
    } catch {
        Write-Host "   ❌ Failed: $_" -ForegroundColor Red
        $failed++
