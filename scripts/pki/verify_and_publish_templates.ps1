...
if (-not $caService) {
    Write-Host "⚠️  Warning: Not running on CA server" -ForegroundColor Yellow
...
foreach ($templateName in $TemplateNames) {
    ...
    if ($exists -and $published) {
        $result.Status = "✅ Ready"
        Write-Host "✅ $templateName - Exists and Published" -ForegroundColor Green
    } elseif ($exists -and -not $published) {
        Write-Host "⚠️  $templateName - Exists but NOT published, publishing..." -ForegroundColor Yellow
        ...
        Write-Host "❌ $templateName - NOT FOUND (needs to be created)" -ForegroundColor Red
    }
...
if ($existingTemplates -eq $totalTemplates) {
    Write-Host "✅ All templates created!" -ForegroundColor Green
} else {
    Write-Host "⚠️  Some templates missing. Create them using:" -ForegroundColor Yellow
...
if ($publishedTemplates -eq $totalTemplates) {
    Write-Host "✅ All templates published to CA!" -ForegroundColor Green
} else {
    Write-Host "⚠️  Some templates not published" -ForegroundColor Yellow
}
...
if ($CreateGroups) {
    ...
    if (-not (Get-ADOrganizationalUnit -Filter "DistinguishedName -eq '$groupsOU'" -ErrorAction SilentlyContinue)) {
        try {
            ...
            Write-Host "✅ Created OU: PKI" -ForegroundColor Green
        } catch {
            Write-Host "⚠️  Could not create PKI OU: $_" -ForegroundColor Yellow
    ...
    foreach ($group in $groups) {
        if (-not (Get-ADGroup -Filter "Name -eq '$($group.Name)'" -ErrorAction SilentlyContinue)) {
            ...
            Write-Host "⚠️  Group already exists: $($group.Name)" -ForegroundColor Yellow
        }
    }
    
    Write-Host ""
    Write-Host "✅ Security groups creation complete" -ForegroundColor Green
}
