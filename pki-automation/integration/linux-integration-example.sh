...
check_prerequisites() {
    ...
    for tool in "${tools[@]}"; do
        if command -v "$tool" &> /dev/null; then
            print_status "âœ“ $tool is installed" "$GREEN"
        else
            print_status "âœ— $tool is not installed" "$RED"
...
install_root_ca() {
    ...
    if curl -s -o "$root_ca_file" "$root_ca_url"; then
        print_status "âœ“ Root CA certificate downloaded" "$GREEN"
    else
        print_status "âœ— Failed to download Root CA certificate" "$RED"
        return 1
    fi
    ...
    if openssl pkcs7 -in "$root_ca_file" -print_certs -out "$root_ca_crt"; then
        print_status "âœ“ Root CA certificate converted" "$GREEN"
    else
        print_status "âœ— Failed to convert Root CA certificate" "$RED"
        return 1
    fi
    ...
    if sudo cp "$root_ca_crt" /etc/pki/ca-trust/source/anchors/; then
        ...
        print_status "âœ“ Root CA certificate installed to trust store" "$GREEN"
    else
        print_status "âœ— Failed to install Root CA certificate" "$RED"
        return 1
    fi
...
EOF
    ...
    if sudo openssl genrsa -out "$key_file" $KEY_SIZE; then
        ...
        print_status "âœ“ Private key generated" "$GREEN"
    else
        print_status "âœ— Failed to generate private key" "$RED"
        return 1
    fi
    ...
    if openssl req -new -key "$key_file" -out "$csr_file" -config "$config_file"; then
        print_status "âœ“ Certificate request generated" "$GREEN"
    else
        print_status "âœ— Failed to generate certificate request" "$RED"
        return 1
    fi
...
submit_certificate_request() {
    ...
    if sudo openssl x509 -req -in "$csr_file" -signkey "/etc/ssl/private/${hostname}.key" \
        ...
        print_status "âœ“ Certificate created: $cert_file" "$GREEN"
    else
        print_status "âœ— Failed to create certificate" "$RED"
        return 1
    fi
...
EOF
        ...
        print_status "âœ“ Apache SSL configuration created" "$GREEN"
...
EOF
        ...
        print_status "âœ“ Nginx SSL configuration created" "$GREEN"
...
EOF
    ...
    print_status "âœ“ SSSD configured for certificate authentication" "$GREEN"
...
test_certificate() {
    ...
    if [[ -f "$cert_file" ]]; then
        ...
        print_status "âœ“ Certificate test completed" "$GREEN"
    else
        print_status "âœ— Certificate file not found: $cert_file" "$RED"
        return 1
    fi
}
