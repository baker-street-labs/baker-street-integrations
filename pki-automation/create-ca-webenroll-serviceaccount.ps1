...
try {
    ...
    Write-Host "`nðŸ“ Checking Organizational Units..." -ForegroundColor Cyan
    ...
    if (-not (Get-ADOrganizationalUnit -Filter "DistinguishedName -eq '$ServiceAccountsOU'" -ErrorAction SilentlyContinue)) {
        Write-Host "ðŸ“‹ Creating ServiceAccounts OU..." -ForegroundColor Yellow
        New-ADOrganizationalUnit -Name "ServiceAccounts" -Path $DomainDN -ProtectedFromAccidentalDeletion $true
        Write-Host "âœ… ServiceAccounts OU created" -ForegroundColor Green
    } else {
        Write-Host "âœ… ServiceAccounts OU exists" -ForegroundColor Green
    }
    ...
    if (-not (Get-ADOrganizationalUnit -Filter "DistinguishedName -eq '$CAServiceOU'" -ErrorAction SilentlyContinue)) {
        Write-Host "ðŸ“‹ Creating CA-Service OU..." -ForegroundColor Yellow
        New-ADOrganizationalUnit -Name "CA-Service" -Path $ServiceAccountsOU -ProtectedFromAccidentalDeletion $true
        Write-Host "âœ… CA-Service OU created" -ForegroundColor Green
    } else {
        Write-Host "âœ… CA-Service OU exists" -ForegroundColor Green
    }
    
    # Check if service account already exists
    Write-Host "`nðŸ” Checking for existing service account..." -ForegroundColor Cyan
    $ExistingAccount = Get-ADUser -Filter "SamAccountName -eq '$AccountName'" -ErrorAction SilentlyContinue
    
    if ($ExistingAccount) {
        Write-Host "âš ï¸  Service account '$AccountName' already exists" -ForegroundColor Yellow
        ...
        Write-Host "`nðŸ”„ Updating existing account..." -ForegroundColor Cyan
        ...
        Write-Host "âœ… Service account updated" -ForegroundColor Green
    } else {
        # Create new service account
        Write-Host "`nâž• Creating new service account..." -ForegroundColor Cyan
    ...
    Write-Host "`nðŸ‘¥ Configuring Security Groups..." -ForegroundColor Cyan
    
    $ServiceAccountsGroup = "Service-Accounts"
    if (-not (Get-ADGroup -Filter "Name -eq '$ServiceAccountsGroup'" -ErrorAction SilentlyContinue)) {
        Write-Host "ðŸ“‹ Creating $ServiceAccountsGroup group..." -ForegroundColor Yellow
        New-ADGroup -Name $ServiceAccountsGroup `
            -GroupScope Global `
            -GroupCategory Security `
            -Path "OU=Groups,$DomainDN" `
            -Description "Service Accounts group"
        Write-Host "âœ… $ServiceAccountsGroup group created" -ForegroundColor Green
    } else {
        Write-Host "âœ… $ServiceAccountsGroup group exists" -ForegroundColor Green
    }
    ...
    if ($GroupMembers -notcontains $AccountName) {
        ...
        Write-Host "âœ… Added $AccountName to $ServiceAccountsGroup" -ForegroundColor Green
    } else {
        Write-Host "âœ… $AccountName is already a member of $ServiceAccountsGroup" -ForegroundColor Green
    }
    
    # Check/Create IIS_IUSRS group membership (for web enrollment)
    Write-Host "`nðŸŒ Configuring Web Enrollment Permissions..." -ForegroundColor Cyan
    
    # Note: IIS_IUSRS is a built-in local group, not AD group
    # We'll need to add this on the server itself
    Write-Host "â„¹ï¸  IIS_IUSRS membership must be configured on the CA server" -ForegroundColor Yellow
    Write-Host "   Run: net localgroup IIS_IUSRS $DomainName\$AccountName /add" -ForegroundColor Yellow
    
    # Verify account creation
    Write-Host "`nâœ… Verifying Service Account..." -ForegroundColor Cyan
    ...
    Write-Host "âš ï¸  Store securely and change after initial configuration" -ForegroundColor Yellow
    
    Write-Host "`nâœ… Service Account Creation Complete!" -ForegroundColor Green
    
} catch {
    Write-Host "`nâŒ Error creating service account: $($_.Exception.Message)" -ForegroundColor Red
