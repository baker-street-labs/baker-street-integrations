...
if curl -k -s "$ACME_DIR_URL" | grep -q "newNonce"; then
    echo "  âœ… StepCA ACME provisioner is accessible"
else
    echo "  âŒ StepCA ACME provisioner not accessible at $ACME_DIR_URL"
...
if [ $? -eq 0 ]; then
    echo "  âœ… IP $ACME_IP is now persistent (survives reboots)"
else
    echo "  âš ï¸  NetworkManager config may have failed, but IP is added manually"
fi
...
if ping -c 2 $ACME_IP > /dev/null 2>&1; then
    echo "  âœ… IP $ACME_IP is reachable"
else
    echo "  âŒ IP $ACME_IP is not reachable"
...
echo "  âœ… Challenge directory created: /var/lib/acme-challenges"
echo ""
...
if [ $? -eq 0 ]; then
    echo "  âœ… ACME responder deployed"
else
...
if [ $? -eq 0 ]; then
    echo "  âœ… ACME responder is running"
else
    echo "  âš ï¸  Pod is not ready yet. Check status with: kubectl get pods -n baker-street-pki"
fi

echo ""
echo "=========================================="
echo "  âœ… ACME Responder Setup Complete!"
echo "=========================================="
echo ""
...
echo "  - Mythic C2:        192.168.0.236:80  âœ… No conflict"
echo "  - ACME Responder:   $ACME_IP:80  âœ… Dedicated"
echo "  - StepCA:           $STEPCA_SERVER:$STEPCA_PORT  âœ… ACME enabled"
echo ""
echo "Next Steps:"
